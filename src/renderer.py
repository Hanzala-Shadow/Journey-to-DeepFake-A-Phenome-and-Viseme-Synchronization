...
from src.face_mesh import (
    ...
    CHIN_MOVERS, STATIC_ANCHORS, MOUTH_OUTER
)
import config
...
def _draw_cavity(canvas: np.ndarray, deformed_lms: list) -> np.ndarray:
    """Draw the dark inside of the mouth behind the stretched lips."""
    h, w = canvas.shape[:2]
    ...
    # Only draw cavity if the mouth is actually open
    if mouth_h < 3:
        return canvas

    cavity = canvas.copy()
    
    # Make cavity slightly narrower than the lip corners so it doesn't bleed out
    ...
    # Feathered mask (make it ultra-soft)
    mask = np.zeros((h, w), dtype=np.uint8)
    cv2.ellipse(mask, (cx, mid_y), (half_w, half_h), 0, 0, 360, 200, -1)  # max opacity 200 instead of 255
    
    fk = max(5, (half_h // 2) * 2 + 1)
    # Double blur for very soft edges
...
def render_frame(base_img: np.ndarray, base_landmarks: list,
                 deformed_landmarks: list, apply_hologram: bool = True,
                 frame_idx: int = 0) -> np.ndarray:
    """
    Render one video frame using Dense Mesh Warping.
    """
    ...
    # 3. Add the dark cavity behind the newly stretched lips
    canvas = _draw_cavity(warped_face, deformed_landmarks)
